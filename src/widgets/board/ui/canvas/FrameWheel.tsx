...
export function FrameWheel({ frames, activeId, onFocus, uiProps }: Props) {
  ...
  return (
    <div className="board-wheel" {...uiProps}>
      <div className="board-wheel-search">
        <Icon name="search" size={12} />
        <input
          className="board-wheel-input"
          value={query}
          placeholder="Фреймы"
          spellCheck={false}
        ...
        >
          ...
          {shown.map((f, i) => {
            ...
            return (
              <button
                key={f.id}
                className={`board-wheel-item${Math.round(center) === i ? ' center' : ''}`}
                style={{ height: ITEM_H, opacity, transform: `scale(${scale})` }}
                title={f.text || 'Фрейм'}
                onClick={() => onItemClick(f.id)}
              >
                <span className="board-wheel-idx">{numberOf.get(f.id)}</span>
                <span className="board-wheel-name">{f.text || 'Фрейм'}</span>
              </button>
            );
          })}
          <div style={{ height: PAD, flexShrink: 0 }} />
