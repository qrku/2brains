...
import styles from './BoardEdges.module.css';
...
interface Props {
  ...
  penWidth: number;
  scale: number;
  onEdgeDown: (e: React.PointerEvent, edge: BEdge, verts: XY[]) => void;
  onBendDown: (e: React.PointerEvent, edgeId: string, index: number, origin: XY) => void;
...
export function BoardEdges({
  ...
  return (
    <svg className={styles['board-svg']} aria-hidden>
        ...
        return (
          <g key={id}>
            <path
              ...
              onPointerDown={(e) => onEdgeDown(e, edge, verts)}
            />
            <path
              d={d}
              stroke={color}
            ...
            {selected &&
              edge.points.map((p, i) => (
                <circle
                  ...
                  onPointerDown={(e) => onBendDown(e, id, i, p)}
