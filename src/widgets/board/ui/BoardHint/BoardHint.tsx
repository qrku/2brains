...
import styles from './BoardHint.module.css';

const HINTS: Record<Tool, React.ReactNode> = {
  cursor: (
    <>
      Потяни — выделить · Клик по блоку — выбрать · Двойной клик — редактировать текст ·{' '}
      <span className={styles['board-hint-dot']}>●</span> — стрелка · Клик по стрелке — выделить,
      ещё раз — изгиб
    </>
  ),
...
export function BoardHint({ tool }: { tool: Tool }) {
  return <div className={styles['board-hint']}>{HINTS[tool]}</div>;
}
