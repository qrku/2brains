...
export function UserPackDetailClient({ id }: Props) {
  ...
  if (!pack) {
    return (
      <div className="container">
        <Link href="/" className="btn-link ghost" style={{ marginBottom: 28, display: 'inline-flex' }}>
          <Icon name="arrow-back" size={12} /> Все паки
        </Link>
        <div className="empty-state" style={{ marginTop: 64 }}>Пак не найден</div>
      </div>
    );
  }
  ...
  return (
    <div className="container">
      <Link href="/" className="btn-link ghost" style={{ marginBottom: 24, display: 'inline-flex' }}>
        <Icon name="arrow-back" size={12} /> Все паки
      </Link>
