...
export function AgentPanel({ open, onClose, chat }: Props) {
  ...
  return (
    <div className={styles['agent-panel']} role="dialog" aria-label="Ассистент">
      <div className={styles['agent-panel-header']}>
        <span className={styles['agent-panel-title']}>Агент</span>
        <span className={styles['agent-panel-context']}>{chat.contextLabel}</span>
        <div style={{ flex: 1 }} />
        <button
          className={styles['agent-panel-icon']}
          title="Очистить переписку"
          onClick={chat.clear}
        >
          <Icon name="edit-01" size={11} />
        </button>
        <button className={styles['agent-panel-icon']} title="Закрыть" onClick={onClose}>
          <Icon name="close" size={11} />
        </button>
      </div>

      <div className={styles['agent-panel-body']} ref={bodyRef}>
        {chat.views.length === 0 && (
          <div className={styles['agent-panel-empty']}>
        ...
        {chat.status === 'error' && chat.errorText && (
          <div className={styles['agent-panel-error']}>{chat.errorText}</div>
        )}
      </div>

      <div className={styles['agent-panel-footer']}>
        <textarea
          ref={inputRef}
          className={styles['agent-panel-input']}
          rows={1}
        ...
        />
        <button
          className={styles['agent-panel-send']}
          title="Отправить"
