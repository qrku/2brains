...
export function spaceSaveNodes(nodes: SpaceNode[], workspaceId: string): void {
  try { localStorage.setItem(wsKey(NODES_KEY, workspaceId), JSON.stringify(nodes)); } catch {}
}

export function spaceReadMeta(workspaceId: string): SpaceMeta {
  try {
    const raw = JSON.parse(localStorage.getItem(wsKey(META_KEY, workspaceId)) ?? '{}') as Partial<SpaceMeta>;
...
export function spaceSaveMeta(meta: SpaceMeta, workspaceId: string): void {
  try { localStorage.setItem(wsKey(META_KEY, workspaceId), JSON.stringify(meta)); } catch {}
}
