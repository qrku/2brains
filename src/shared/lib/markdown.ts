...
function slug(s: string): string {
  return s.toLowerCase().replace(/[^\wа-яёa-z0-9]+/gi, '-').replace(/^-+|-+$/g, '');
}
...
function parseTable(lines: string[]): string {
  ...
  const tbody = lines.slice(2).map((row) =>
    `<tr>${splitRow(row).map((c, i) => `<td class="md-td"${style(i)}>${parseInline(c)}</td>`).join('')}</tr>`
  ).join('');
...
function isULI(s: string): boolean { return /^[-*+]\s/.test(s.trimStart()); }
function isOLI(s: string): boolean { return /^\d+[.)]\s/.test(s.trimStart()); }
function getIndent(s: string): number { return s.match(/^(\s*)/)?.[1].length ?? 0; }

function stripULI(s: string): string { return s.trimStart().replace(/^[-*+]\s/, ''); }
function stripOLI(s: string): string { return s.trimStart().replace(/^\d+[.)]\s/, ''); }
...
function buildListItems(lines: string[], baseIndent: number, ordered: boolean): string {
  ...
  while (i < lines.length) {
    ...
    if (indent > baseIndent) { i++; continue; } // shouldn't happen if called correctly
  ...
  while (i < lines.length) {
    ...
    if ((trim.startsWith('|') || trim.includes('|')) && i + 1 < lines.length) {
      ...
      if (/^\|?[-:|\s]+\|/.test(sep)) {
        ...
        if (tbl.length >= 2) { out.push(parseTable(tbl)); continue; }
    ...
    while (i < lines.length) {
      ...
      if (i + 1 < lines.length) {
        ...
        if (/^=+$/.test(nl) || /^-{2,}$/.test(nl)) { para.push(l); i++; break; }
      }
      para.push(l);
      i++;
    }
